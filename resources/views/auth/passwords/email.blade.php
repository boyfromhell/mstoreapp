...
<div class="flex flex-wrap items-center py-32 px-4 py-32 md:px-0">
    <div class="w-full max-w-md md:mx-auto">
        <div class="rounded shadow">
            <div class="font-base text-base bg-mstore text-white p-1 rounded-t">
                {{ __('Reset Password') }}
            </div>
            <div class="bg-black p-3 rounded-b">
                @if (session('status'))
                    <div class="bg-green-lightest border border-mstore-light text-mstore-dark text-sm px-4 py-3 rounded mb-4">
                        {{ session('status') }}
                    </div>
                @endif
                ...
                <form class="form-horizontal" method="POST" action="{{ route('password.email') }}">
                    ...
                    <div class="flex">
                        <div class="w-3/4 ml-auto">
                            <button type="submit" class="bg-mstore text-grey-lighter hover:text-white text-sm font-semibold py-2 px-4 rounded mr-3">
